...
// Save one-time organiser message to candidate application.
export async function saveOrganiserMessage({ applicationId, mobile, message }) {
  const db = getAppDb()
  const cleanAppId = String(applicationId || '').trim().toUpperCase()
  const cleanMobile = String(mobile || '').replace(/\D/g, '').slice(-10)
  const filter = cleanAppId ? { application_id: cleanAppId } : { mobile: cleanMobile }
