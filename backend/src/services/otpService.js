...
export async function sendOtp(mobile) {
  ...
  const demoOtp = process.env.DEMO_TEST_OTP
  const apiKey = process.env.SMS_API_KEY
  const template = process.env.SMS_TEMPLATE_NAME || 'OTP1'

  if (!apiKey && (demoOtp || process.env.OTP_DEV_BYPASS === '1')) {
...
export async function verifyOtp(mobile, otp) {
  ...
  const demoOtp = process.env.DEMO_TEST_OTP
  if ((demoOtp && code === String(demoOtp)) || (process.env.OTP_DEV_BYPASS === '1' && code === '123456')) {
    sessions.delete(m)
...
export function devBypassEnabled() {
  return process.env.OTP_DEV_BYPASS === '1' || Boolean(process.env.DEMO_TEST_OTP)
}
