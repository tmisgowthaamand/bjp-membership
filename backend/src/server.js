...
function validateEnv() {
  const missing = ['MONGO_VOTER_URL', 'MONGO_APP_URL'].filter((k) => !process.env[k])
  if (!process.env.SMS_API_KEY && !process.env.DEMO_TEST_OTP && process.env.OTP_DEV_BYPASS !== '1') {
    missing.push('SMS_API_KEY (or DEMO_TEST_OTP / OTP_DEV_BYPASS)')
  }
  if (missing.length) {
